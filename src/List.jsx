...
export default function List() {
    ...
    const filteredBooks = books.filter(book => {
        ...
        const title = book.title.toLowerCase().includes(lowercasedSearch);
        const author = book.author.toLowerCase().includes(lowercasedSearch);

        return title || author;
    });

    const listitems = filteredBooks.map((book) =>
        <li key={book.id}>
    ...
    return (
        <div className="list-container">
            ...
            />
            <ul className="list-books">{listitems}</ul>
        </div>
    );
}